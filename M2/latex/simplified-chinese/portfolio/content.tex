...
\midrule
\hyperlink{us-port-01}{US-PORT-01} & 探索并保存 Model Portfolio & \texttt{Recommended} -\textgreater{} \texttt{Model Portfolio} -\textgreater{} \texttt{Watchlist} /\allowbreak{} \texttt{My Portfolio} \\
\hyperlink{us-port-02}{US-PORT-02} & 创建、编辑和维护 My Portfolio & \texttt{My portfolios} -\textgreater{} \texttt{Edit} /\allowbreak{} \texttt{Baseline snapshots} \\
...
\hyperlink{us-port-04}{US-PORT-04} & 分析 Portfolio 并审阅 AI 建议 & \texttt{Draft Portfolio} -\textgreater{} \texttt{Analysis} -\textgreater{} \texttt{Suggestions} \\
\hyperlink{us-port-05}{US-PORT-05} & 根据 Baseline Snapshot 审阅 & \texttt{Portfolio Review} -\textgreater{} \texttt{Baseline} -\textgreater{} \texttt{Analysis} -\textgreater{} \texttt{Suggestions} \\
\bottomrule
\end{longtable}
\endgroup
\section{统一术语}
