...
> 范围说明：`Google`、`Microsoft` 和 `Sign in with Apple` 虽然显示在 `Login` 页面，但不属于本次可验收的工作路径；视频中只需交代边界，不要演示为已完成能力。

## User Story / Video 索引

| Story | 视频主题 | 核心路径 | Video |
| --- | --- | --- | --- |
| [US-AUTH-01](#user-story-1---注册新账号并使用已有账号登录) | Authentication Flow | `Register New Account` + email/password `Sign In` | [视频](https://jjpvro70sief.jp.larksuite.com/wiki/CDmcwSmsNinDn8kb92Nj980Epne) |
...
*AUTH-US1-01：`Login` 页面默认态，包含 account/email 与 password 输入框、`Sign In`、`Register New Account` 和三种其他登录入口。*
...
*AUTH-US1-02：`Create Account` 有效表单完成态，包含 display name、已脱敏的 demo email、隐藏的 password，以及已启用的 `Create Account`。*
...
*AUTH-US1-03：`Verify Email` 初始态，显示已脱敏的收件 email、空 code 输入框，以及 code 未满六位时不可用的 `Verify and Continue`。六位 code 完成态及提交操作由视频连续展示。*
...
*AUTH-US1-04：新账号验证成功后进入 `Markets` landing page；底部导航保留在画面中，用于证明已离开认证流程。*
...
> **视频证据：** 已有账号的 signing-in/loading state、重复提交保护和认证成功后的跳转属于短暂交互状态，由视频连续展示；静态截图以本 story 已归档的四个关键页面为准。
...
- [ ] `AUTH-US1-AC05` `Google`、`Microsoft` 和 `Sign in with Apple` 不被本视频表述为本版本已验收的工作路径。
...
| ID | 已归档画面 | 文件名 |
| --- | --- | --- |
| AUTH-US1-01 | `Login` 完整默认态 | `auth-us1-01-login-page.png` |
| AUTH-US1-02 | `Create Account` 有效表单（email 已脱敏） | `auth-us1-02-create-account.png` |
| AUTH-US1-03 | `Verify Email` 初始/按钮不可用状态 | `auth-us1-03-verify-email.png` |
| AUTH-US1-04 | 注册成功后的 `Markets` | `auth-us1-04-registration-success.png` |
